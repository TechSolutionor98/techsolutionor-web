...
const ChallengeAccepted = () => {
  ...
  return (
    <div className='relative mt-20'>
      ...
      <div className="cards bg-[#262323] w-full h-[500px] mt-20">
        {/* IF Image of stars wll come */}
        <div className="challaenge-cards flex items-center justify-center h-full px-5 md:px-10 gap-5 md:gap-5 ">
            {challenges.map((item, i)=>(
                <div key={i} className={`card w-[350px] h-[450px] rounded-[20px] flex flex-col items-start -mt-40  px-7 py-5 text-black bg-white`} style={{backgroundImage: `url(${CardsBg.src})`, backgroundSize: 'cover', boxShadow: ' 0px 7px 8px 0px rgba(0, 0, 0, 0.5)'}}>
                    <h1 className='text-[30px] font-[600] leading-[30px] text-center'>{item.title}</h1>
                    <p className='text-[15px] mt-3 leading-[20px] '>{item.desc}</p>
                    <ul className='list-disc text-[15px] mt-3 leading-[20px]'>
